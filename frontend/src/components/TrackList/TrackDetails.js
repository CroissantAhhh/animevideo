...
import { usePlaylistPopout } from "../../context/PlaylistPopout";
import { useSelector, useDispatch } from "react-redux";
import { useState } from 'react';
import { Link } from 'react-router-dom';
import { addTrackToPlaylist, removeTrackFromPlaylist } from "../../store/playlist";
import { removeTrack } from "../../store/tracks";
import { PlaylistPopout } from "../../context/PlaylistPopout";
import AddPlaylistPopout from "./AddPlaylistPopout";
...
const TrackDetails = ({ track }) => {
    ...
    function favoriteTrack(playlist) {
        if (songInYourSongs) {
            setSongInYourSongs(false);
            dispatch(removeTrackFromPlaylist(playlist.id, track.id));
            if (URL.includes("playlists")) {
                dispatch(removeTrack(track.id))
            }
        } else {
            setSongInYourSongs(true);
            dispatch(addTrackToPlaylist(playlist.id, track.id));
        }
    };

    function clickAddTrack(e) {
        ...
        <div className="track-section">
            ...
            <div className="track-section-art-play">
                ...
                <div className="track-section-buttons">
                    <button className={"add-track" + (songInYourSongs ? " in-your-songs" : "")}  onClick={() => favoriteTrack(yourSongsPlaylist)} style={{display: `${sessionUser ? "block" : "none" }`}}>
                    ...
                    {/* <button className="add-track-playlist" onClick={() => setShowPopout(true)} style={{display: `${sessionUser ? "block" : "none" }`}}>
                    ...
                    <AddPlaylistPopout track={track}>LET ME SEE THIS</AddPlaylistPopout>
                </div>
            </div>
        </div>
    );
